...
def convert_list_ints(seq: Sequence[int]) -> str:
    return "[{}]".format(", ".join(str(v) for v in seq))
...
class EncounterSlot:
    ...
    def __str__(self) -> str:
        ret = f"EncounterSlot(species=\"{self.species}\""
...
class Move:
    ...
    def __str__(self) -> str:
        ret = f"Move(id={self.id}, type=PokemonType.{self.type.upper()}"
    ...
    def validate(self):
        ...
        for encounter in self.encounters.values():
            for type in encounter_types:
                for slot in getattr(encounter, type):
                    if type == "rock_smash":
                        assert not slot.accessibility, "rock smash has no accessibility"
        ...
        for spec in self.species.values():
            ...
            if spec.pre_evolution is not None:
                ...
                if mthd in {
                    ...
                    assert pe.level is None and pe.item is None and pe.move is not None and pe.other_species is None, f"only move for method {mthd}"

...
def set_rules(world: "PokemonHgssWorld") -> None:
    ...
    for name, rule in rules.trainer_rules.items():
        if name.startswith("rival_"):
            parent_region = trainerdata.trainers[name + "_chikorita"].parent_region
        else:
    ...
    done_encs = set()
    slot_version = "hg" if world.options.version.value == VersionEnum.HEARTGOLD else "ss"
    enc_acc = world.options.in_logic_encounters.methods()
    for region_name, region_data in regiondata.regions.items():
        enc_key = region_data.encounters
        ...
        for type in sorted(region_data.accessible_encounters):
            ...
            if (enc_key, type) not in done_encs:
                ...
                if type == "rock_smash":
                    ...
                    for i, slot in enumerate(e):
                        if slot.in_logic(world.options.version.value, enc_acc):
                            world.set_rule(world.multiworld.get_location(f"{enc_key}_{type}_{i + 1}", world.player), rules.get_enc_accessibility_rule(slot.accessibility))
        ...
        while True:
            ...
            while hms_to_verify:
                ...
                if not rsvds[hm_to_verify]._evaluate(state):
                    ...
                    print(world.accessible_mons, valid_pokemon)
